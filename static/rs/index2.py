...
def mark(the_id):
  sel.chselect(the_id)
  deselect(sel.bef_select)
  select(sel.curr_select)
  checkFrame()
  moveFrame()
  scrollToFrame()
...
def moveFrame():
  ...
  if sel.curr_num() >= top_frame and sel.curr_num() <= bot_frame:
    pass
  else:
    if sel.curr_num() > bot_frame:
      bot_frame = sel.curr_num()
      top_frame = bot_frame - 7
    elif sel.curr_num() < top_frame:
      top_frame = sel.curr_num()
      bot_frame = top_frame + 7
...
def upOrDown(e):
  ...
  if (e.keyCode == 38):
    if (sel.curr_num() != 1):
      ch_mark(sel.curr_num() - 1)
    else:
      mark(sel.curr_select)
  elif (e.keyCode == 40):
    if (sel.curr_num() != numberOfRows):
      ch_mark(sel.curr_num() + 1)
    else:
      mark(sel.curr_select)
  elif (e.keyCode == 13):
...
def reaccItem():
  emptyTheFields()
  copyDataToTemp(sel.curr_num()-1)
  removeARow(sel.curr_select)
  entriedItem.splice(sel.curr_num()-1,1)
